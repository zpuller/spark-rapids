...
/**
 * Base class for the GPU version of Databricks Delta Lake DeleteCommand.
 *
 * Performs a Delete based on the search condition.
 *
 * Algorithm:
 *   1) Scan all the files and determine which files have rows that need to be deleted.
 ...
 */
abstract class GpuDeleteCommandBase(
  ...
  final override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    recordDeltaOperation(gpuDeltaLog.deltaLog, "delta.dml.delete") {
      ...
      recacheByPlan(sparkSession, target)
    }

    // Adjust for deletes at partition boundaries. Deletes at partition boundaries is a metadata
    // operation, therefore we don't actually have any information around how many rows were
    // deleted.
    // While this info may exist in the file statistics, it's not guaranteed that we have these
    // statistics. To avoid any performance regressions, we currently just return a -1 in such
    // cases.
      ...
      val dfToWrite = if (shouldWriteCdc) {
        ...
        baseData
            ...
            .withColumn(
              ...
              exprToColumn(If(filterCondition, CDC_TYPE_NOT_CDC, CDC_TYPE_DELETE))
            )
      } else {
        baseData
            .filter(numTouchedRowsUdf())
            .filter(exprToColumn(filterCondition))
      }
  ...
  private def shouldWritePersistentDeletionVectors(txn: OptimisticTransaction): Boolean = {
    conf.getConf(DeltaSQLConf.DELETE_USE_PERSISTENT_DELETION_VECTORS) &&
      DeletionVectorUtils.deletionVectorsWritable(txn.snapshot)
  }

  private def createDataFrame(sparkSession: SparkSession, plan: LogicalPlan): DataFrame = {
    TrampolineConnectShims.createDataFrame(
      sparkSession.asInstanceOf[TrampolineConnectShims.SparkSession], plan)
  }

  private def exprToColumn(expr: Expression): Column = DFUDFShims.exprToColumn(expr)

  private def recacheByPlan(sparkSession: SparkSession, plan: LogicalPlan): Unit = {
    val classic = sparkSession.asInstanceOf[TrampolineConnectShims.SparkSession]
