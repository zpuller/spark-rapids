...
/**
 * GPU implementation for TimestampAddInterval (Spark 4.1+).
 * This is the renamed version of TimeAdd from earlier Spark versions.
 */
case class GpuTimestampAddInterval(start: Expression,
                                   interval: Expression,
        ...
        (lhs, rhs) match {
            ...
            val interval = intervalS.dataType match {
              ...
              case _ =>
                throw new UnsupportedOperationException(
                  "GpuTimestampAddInterval unsupported data type: " + intervalS.dataType)
            }
          ...
          case (l, r: GpuColumnVector) =>
            (l.dataType(), r.dataType) match {
              ...
              case _ =>
                throw new UnsupportedOperationException(
                  "GpuTimestampAddInterval takes column and interval as an argument only")
            }
          case _ =>
            throw new UnsupportedOperationException(
              "GpuTimestampAddInterval takes column and interval as an argument only, the types " +
